...
class dragonnest(commands.Cog):
	...
	async def dnfeed(self, ctx):
		if ctx.invoked_subcommand is None:
			await ctx.send("the `!dnfeed` command lets you customize/remove and relocate your Dragon Nest feed channel(only usable by admins)\n`!dnfeed off` < turns dnfeed message off\n`!dnfeed on` < turns on dnfeed channel and applies default settings, unless you had something previously saved\n`!dnfeed edit` < makes your custom dnfeed message, limit of 1.5k characters. type `!dnfeed edit` to see the format!\n`!dnfeed here` < allocates the dnfeed text to the text channel you invoked the command in\n`!dnfeed check` < shows your saved or default dnfeed message")

	@dnfeed.command(aliases=["off","disable"])
	async def D_off(self, ctx):
		...
		if sid not in joins['disabled']:
			...
			await ctx.send("DNFeed message disabled")
		else:
			await ctx.send("DNFeed message is already disabled")

	@dnfeed.command(aliases=["on","enable"])
	async def D_on(self, ctx):
		sid = str(ctx.message.guild.id)
		if sid in joins['disabled']:
			joins['disabled'].remove(sid)
			with open('C:/DISCORD BOT/DiscordStuff/joins.json', 'w') as f:
				json.dump(joins, f, indent = 4)
			await ctx.send("DNFeed message enabled")
		else:
			await ctx.send("DNFeed message is already enabled")

	@dnfeed.command(aliases=["this","here"])
	async def D_here(self, ctx):
		sid = str(ctx.message.guild.id)
		mid = str(ctx.message.channel.id)
		if joins.get(sid) is None:
			joins[sid] = {}
		if joins[sid].get("channel") == mid:
			await ctx.send("The DNFeed channel is already set to this channel.")
		else:
			joins[sid]["channel"] = mid
			with open('C:/DISCORD BOT/DiscordStuff/joins.json', 'w') as f:
				json.dump(joins, f, indent = 4)
			await ctx.send("DNFeed message channel allocated to {}".format(ctx.message.channel.name))

	@dnfeed.command(aliases=["edit","modify","change"])
	async def D_edit(self, ctx, *, wm = None):
		sid = str(ctx.message.guild.id)
		if joins.get(sid) is None:
			joins[sid] = {}
		if wm == None:
	...
	@dnfeed.command(aliases=["show","check","message"])
	async def D_message(self, ctx):
		sid = str(ctx.message.guild.id)
