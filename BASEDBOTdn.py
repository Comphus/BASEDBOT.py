...
class dragonnest:
	...
	async def skillbuilds(self, ctx):
		await self.bot.say(self.sbs(ctx.message))
						...
						try:
							if after.game.type == 1:
								...
								if rtLimit[after.id] == None or (datetime.now() - rtLimit[after.id]).total_seconds() // 3600 >= 2:
									...
									r = requests.get('https://api.twitch.tv/kraken/streams/{}/?&client_id=4asyzu8i1l7ea1f61aebw3mgbuv04y2'.format(after.game.url.replace('https://www.twitch.tv/', '')))
									tData = r.json()
